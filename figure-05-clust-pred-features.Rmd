...
mat_full <-
  df_varsel %>%
  ## Remove features that appear for fewer than 3 weeks.
...
## Cluster longitudinal data using sparse Fréchet distance.
df_frechet <-
  df_varsel %>%
  ## Remove features that appear for fewer than 3 weeks.
  inner_join(df_varsel %>%
               ...
               dplyr::select(-n),
             by = "ranking_fulldata") %>%
  ## dist.array needs at least 3 replicates, so expand cv_proportions_diag.
  group_by(ranking_fulldata, week, size) %>%
  mutate(cv_proportions_diag = cv_proportions_diag * 5L) %>%
  uncount(cv_proportions_diag) %>%
  mutate(rep =
           row_number() %>%
           factor(levels = 1:5)) %>%
  ...
  ungroup() %>%
  right_join(x = .,
             y = expand(data = select(., -size), week, ranking_fulldata, rep),
             by = c("week", "ranking_fulldata", "rep")) %>%
  replace_na(list(size = 0L)) %>%
  pivot_wider(id_cols = c(ranking_fulldata, rep),
              names_from = week,
              values_from = size,
              values_fill = 0L) %>%
  ## dist.array needs specific names of columns.
  rename(ID = ranking_fulldata) %>%
  pivot_longer(-c(ID, rep),
               names_to = "Time",
               values_to = "Variable1") %>%
  mutate(Time = as.integer(Time),
         ## dist.array() assumes at least 2 variables.
         Variable2 = 0L) %>%
  arrange(ID, Time, rep) %>%
  select(-rep) %>%
  as.data.frame()
...
n_clusters <-
  clust_f$Cluster$Cluster |>
  as.integer() |>
  max()
## colors_cluster <-
##   palette.colors(n_clusters + 2, palette = "Okabe-Ito")[-1] %>%
##   setNames(c("3", "1", "2"))
## labels_cluster <-
##   c("1" = "Formation",
##     "2" = "Maintenance",
...
ggheatmap(mat_full, filling = NULL) +
  ...
  align_dendro(k = 2,
               distance =
                 dist_f[, , 1] |>
                 `dimnames<-`(list(clust_f$Cluster$ID,
                                   clust_f$Cluster$ID)) |>
                 as.dist(),
               aes(color = branch)) +
