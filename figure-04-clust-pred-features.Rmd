...
## library(shadowtext) # geom_shadowtext
```
...
```{r figure-04-clust-pred-features}
#| cache: true
#| dependson: -1
#| fig.width: 9
#| fig.height: 5.2
...
names_map <- c(
  ...
  "t_gam_Children_t_gams_Count_mean" =
    "Number of T cells activating macrophages"
)
## ## Use the original column names if unlabelled above.
## names(names_map)[names(names_map) == ""] <-
##   names_map[names(names_map) == ""] %>%
##   setNames(NULL)
...
ggheatmap(mat_full, filling = NULL) +
  ...
  ##   size = 7## ,
  ##   ## face = ifelse(str_detect(rownames(mat_full), "^mac"), "bold", "plain")
  ## )) +
  labs(x = "Week",
       y = "Spatial feature",
       fill = "Rank") +
  ...
  ## Overlay rank value labels instead of a hard-to-read gradient.
  ## guides(fill = "none") +
  ## "value" is generated by ggheatmap() per
  ## https://yunuuuu.github.io/ggalign-book/heatmap-layout.html#sec-heatmap-layout-main-plot
  ## geom_shadowtext(aes(label = value), size = 3) +
