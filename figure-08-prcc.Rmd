...
## Maxmimize color contrast between significant lines that cross.
##
...
##
## First make sure the constraints are solvable.
...
alldifferent <-
  Rglpk_solve_LP(obj = rep(1L, n_params_signif * n_colors),
                 ...
                 rhs = c(rep(0L, nrow(constraints_intersect)),
                         rep(1L, nrow(constraints_one_color))),
                 types = rep("B", n_params_signif * n_colors))
## stopifnot(alldifferent$status == 0)

## Now maximize the hue differences.
## max_hue_diff <-
##   Rglpk_solve_LP(...)
